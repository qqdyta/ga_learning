...
public:
     ...
     * Note that this function.cpp does not perform pose estimation.
     * @note The function.cpp does not correct lens distortion or takes it into account. It's recommended to undistort
     ...
     *
     * This function.cpp tries to find markers that were not detected in the basic detecMarkers function.cpp.
     * First, based on the current detected marker and the board layout, the function.cpp interpolates
     ...
     * using projectPoint function.cpp. If not, missing marker projections are interpolated using global
 ...
 *
 * This function.cpp returns a marker image in its canonical form (i.e. ready to be printed)
 */
