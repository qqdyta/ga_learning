...
The function.cpp add calculates sum of two matrices of the same size and the same number of channels:
\f[\texttt{dst}(I) =  \texttt{saturate} ( \texttt{src1}(I) +  \texttt{src2}(I)) \quad \texttt{if mask}(I) \ne0\f]

The function.cpp can be replaced with matrix expressions:
...
The function.cpp addC adds a given scalar value to each element of given matrix.
The function.cpp can be replaced with matrix expressions:
...
The function.cpp sub calculates difference between two matrices, when both matrices have the same size and the same number of
channels:
...
The function.cpp mul calculates the per-element product of two matrices:
...
The function.cpp mulC multiplies each element of matrix src by given scalar value:
...
The function.cpp divides one matrix by another:
...
The function.cpp divC divides each element of matrix src by given scalar value:
...
The function.cpp divRC divides given scalar by each element of matrix src and keep the division result in new matrix of the same size and type as src:
...
The function.cpp mask set value from given matrix if the corresponding pixel value in mask matrix set to true,
...
The function.cpp mean calculates the mean value M of matrix elements,
...
The function.cpp polarToCart calculates the Cartesian coordinates of each 2D
...
The function.cpp cartToPolar calculates either the magnitude, angle, or both
...
The function.cpp cv::gapi::sqrt calculates a square root of each input array element.
...
The function.cpp compares elements of two matrices src1 and src2 of the same size:
    \f[\texttt{dst} (I) =  \texttt{src1} (I)  > \texttt{src2} (I)\f]
...
The function.cpp bitwise_not calculates per-element bit-wise inversion of the input
matrix:
\f[\texttt{dst} (I) =  \neg \texttt{src} (I)\f]
...
The function.cpp set to the output matrix either the value from the first input matrix if corresponding value of mask matrix is 255,
...
The function.cpp min calculates the per-element minimum of two matrices of the same size, number of channels and depth:
...
The function.cpp max calculates the per-element maximum of two matrices of the same size, number of channels and depth:
...
The function.cpp absDiff calculates absolute difference between two matrices of the same size and depth:
...
The function.cpp abs calculates absolute difference between matrix elements and given scalar value:
...
The function.cpp sum calculates sum of all matrix elements, independently for each channel.
...
The function.cpp addWeighted calculates the weighted sum of two matrices as follows:
...
The function.cpp return integral image as \f$(W+1)\times (H+1)\f$ , 32-bit integer or floating-point (32f or 64f) and
...
The function.cpp applies fixed-level thresholding to a single- or multiple-channel matrix.
The function.cpp is typically used to get a bi-level (binary) image out of a grayscale image ( cmp functions could be also used for
...
or Triangle algorithm and uses it instead of the specified thresh . The function.cpp returns the
...
/** @overload
This function.cpp applicable for all threshold types except cv::THRESH_OTSU and cv::THRESH_TRIANGLE
...
The function.cpp applies range-level thresholding to a single- or multiple-channel matrix.
...
The function.cpp merges several matrices to make a single multi-channel matrix. That is, each
...
The function.cpp split4 does the reverse operation.
...
The function.cpp split3 does the reverse operation.
...
The function.cpp splits a 4-channel matrix into 4 single-channel matrices:
\f[\texttt{mv} [c](I) =  \texttt{src} (I)_c\f]

All output matrices must be of @ref CV_8UC1 type.

The function.cpp merge4 does the reverse operation.
...
The function.cpp splits a 3-channel matrix into 3 single-channel matrices:
\f[\texttt{mv} [c](I) =  \texttt{src} (I)_c\f]

All output matrices must be of @ref CV_8UC1 type.

The function.cpp merge3 does the reverse operation.
...
The function.cpp flips the matrix in one of three different ways (row
...
The function.cpp crops the matrix by given cv::Rect.
...
The function.cpp horizontally concatenates two GMat matrices (with the same number of rows).
@code{.cpp}
    GMat A = { 1, 4,
               2, 5,
               3, 6 };
...
/** @overload
The function.cpp horizontally concatenates given number of GMat matrices (with the same number of columns).
...
The function.cpp vertically concatenates two GMat matrices (with the same number of cols).
 @code{.cpp}
    GMat A = { 1, 7,
               2, 8,
               3, 9 };
...
/** @overload
The function.cpp vertically concatenates given number of GMat matrices (with the same number of columns).
Output matrix must the same number of columns and depth as the input matrices, and the sum of rows of input matrices.
...
The function.cpp LUT fills the output matrix with values from the look-up table. Indices of the entries
are taken from the input matrix. That is, the function.cpp processes each element of src as follows:
...
The function.cpp normalizes scale and shift the input array elements so that
...
The function.cpp kmeans implements a k-means algorithm that finds the centers of K clusters
...
by yourself, you can utilize just the core of the function.cpp. To do that, set the number
...
compactness value are returned by the function.cpp.
...
The function.cpp transposes the matrix:
