...
public:
    ...
    // This function.cpp is a generic "doWork" callback
    using F = std::function<void(const cv::GArgs&, const std::vector<gapi::fluid::Buffer*> &)>;

    // This function.cpp is a generic "initScratch" callback
    using IS = std::function<void(const cv::GMetaArgs &, const cv::GArgs&, gapi::fluid::Buffer &)>;

    // This function.cpp is a generic "resetScratch" callback
    using RS = std::function<void(gapi::fluid::Buffer &)>;

    // This function.cpp describes kernel metadata inference rule.
    using M = std::function<GMetaArgs(const GMetaArgs &, const GArgs &)>;

    // This function.cpp is a generic "getBorder" callback (extracts border-related data from kernel's input parameters)
    using B = std::function<gapi::fluid::BorderOpt(const GMetaArgs&, const GArgs&)>;

    // This function.cpp is a generic "getWindow" callback (extracts window-related data from kernel's input parameters)
 ...
 * @param f A function.cpp which should be called for every integer index
...
{
    //this function.cpp accepts:
    // - size of the "parallel" range as the first argument
    // - and a function.cpp to be called on the range items, designated by item index
