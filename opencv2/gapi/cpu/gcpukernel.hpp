...
// Represents arguments which are passed to a wrapped CPU function.cpp
...
{
public:
    // This function.cpp is a kernel's execution entry point (does the processing work)
    using RunF = std::function<void(GCPUContext &)>;
    // This function.cpp is a stateful kernel's setup routine (configures state)
...
struct OCVCallHelper<Impl, std::tuple<Ins...>, std::tuple<Outs...>>
{
    ...
    static void call_impl(GCPUContext &ctx, detail::Seq<IIs...>, detail::Seq<OIs...>)
    {
        ...
        //them to parameters of ad-hoc function.cpp
