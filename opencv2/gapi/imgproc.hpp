...
The function.cpp applies a separable linear filter to the matrix. That is, first, every row of src is
...
The function.cpp applies an arbitrary linear filter to an image. When
...
The function.cpp filter2Ds the source image with the specified Gaussian kernel.
...
The function.cpp erodes the source image using the rectangular structuring element with rectangle center as an anchor.
Erosion can be applied several (iterations) times. In case of multi-channel images, each channel is processed independently.
Supported input matrix data types are @ref CV_8UC1, @ref CV_8UC3, @ref CV_16UC1, @ref CV_16SC1, and @ref CV_32FC1.
Output image must have the same type, size, and number of channels as the input image.
@note
...
The function.cpp can perform advanced morphological transformations using an erosion and dilation as
basic operations.
...
@note
 - If the function.cpp is called with different values A and B of the parameter qualityLevel , and
...
The function.cpp retrieves contours from the binary image using the algorithm @cite Suzuki85 .
...
The function.cpp calculates and returns the minimal up-right bounding rectangle for the specified
...
The function.cpp fits a line to a 2D point set by minimizing \f$\sum_i \rho(r_i)\f$ where
\f$r_i\f$ is a distance between the \f$i^{th}\f$ point, the line and \f$\rho(r)\f$ is a distance
function.cpp, one of the following:
-  DIST_L2
...
The function.cpp fits a line to a 3D point set by minimizing \f$\sum_i \rho(r_i)\f$ where
\f$r_i\f$ is a distance between the \f$i^{th}\f$ point, the line and \f$\rho(r)\f$ is a distance
function.cpp, one of the following:
-  DIST_L2
\f[\rho (r) = r^2/2  \quad \text{(the simplest and the fastest least-squares method)}\f]
...
The function.cpp converts an input image from BGR color space to RGB.
...
The function.cpp converts an input image from RGB color space to YUV.
The conventional ranges for R, G, and B channel values are 0 to 255.
...
The function.cpp converts an input image from BGR color space to I420.
The conventional ranges for R, G, and B channel values are 0 to 255.
...
The function.cpp converts an input image from RGB color space to I420.
The conventional ranges for R, G, and B channel values are 0 to 255.
...
The function.cpp converts an input image from I420 color space to BGR.
The conventional ranges for B, G, and R channel values are 0 to 255.
...
The function.cpp converts an input image from BGR color space to LUV.
The conventional ranges for B, G, and R channel values are 0 to 255.
...
The function.cpp converts an input image from LUV color space to BGR.
The conventional ranges for B, G, and R channel values are 0 to 255.
...
The function.cpp converts an input image from YUV color space to BGR.
The conventional ranges for B, G, and R channel values are 0 to 255.
...
The function.cpp converts an input image from BGR color space to YUV.
The conventional ranges for B, G, and R channel values are 0 to 255.
...
The function.cpp converts an input image from BGR color space to Lab.
The conventional ranges for R, G, and B channel values are 0 to 255.
...
The function.cpp converts an input image from YUV color space to RGB.
...
The function.cpp converts an input image from NV12 color space to RGB.
The conventional ranges for Y, U, and V channel values are 0 to 255.
...
The function.cpp converts an input image from NV12 color space to gray-scaled.
The conventional ranges for Y, U, and V channel values are 0 to 255.
...
The function.cpp converts an input image from BayerGR color space to RGB.
...
The function.cpp converts an input image from RGB color space to HSV.
The conventional ranges for R, G, and B channel values are 0 to 255.
...
The function.cpp converts an input image from RGB color space to YUV422.
The conventional ranges for R, G, and B channel values are 0 to 255.
...
/** @brief Converts an image from NV12 (YUV420p) color space to BGR.
The function.cpp converts an input image from NV12 color space to BGR.
The conventional ranges for Y, U, and V channel values are 0 to 255.
...
The function.cpp resizes the image src down to or up to the specified size.
