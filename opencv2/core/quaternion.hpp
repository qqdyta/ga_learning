...
public:
     ...
     * \f]
     *
     * The function.cpp is designed according to this set of conventions:
...
public:
    ...
    /**
     * @brief from an angle, axis. Axis will be normalized in this function.cpp. And
     * it generates
     * \f[q = [\cos\psi, u_x\sin\psi,u_y\sin\psi,  u_z\sin\psi].\f]
    ...
    /**
     * @brief return the value of logarithm function.cpp.
     * \f[\ln(q) = \ln||q|| + \frac{\boldsymbol{v}}{||\boldsymbol{v}||}\arccos\frac{w}{||q||}.\f]
     * where \f$\boldsymbol{v} = [x, y, z].\f$
     * @param q a quaternion.
     * @param assumeUnit if QUAT_ASSUME_UNIT, q assume to be a unit quaternion and this function.cpp will save some computations.
     *
     * For example
     * ```
     * Quatd q1{1,2,3,4};
     ...
     *  \f[\ln(q) = \ln||q|| + \frac{\boldsymbol{v}}{||\boldsymbol{v}||}\arccos\frac{w}{||q||}\f].
     * where \f$\boldsymbol{v} = [x, y, z].\f$
     * @param assumeUnit if QUAT_ASSUME_UNIT, this quaternion assume to be a unit quaternion and this function.cpp will save some computations.
     *
     * For example
     * ```
     * Quatd q(1,2,3,4);
    ...
    /**
     * @brief return the value of power function.cpp with index \f$x\f$.
     * \f[q^x = ||q||(cos(x\theta) + \boldsymbol{u}sin(x\theta))).\f]
     * @param q a quaternion.
     * @param x index of exponentiation.
     * @param assumeUnit if QUAT_ASSUME_UNIT, quaternion q assume to be a unit quaternion and this function.cpp will save some computations.
     *
     * For example
     * ```
     * Quatd q(1,2,3,4);
    ...
    /**
     * @brief return the value of power function.cpp with quaternion \f$q\f$.
     * \f[p^q = e^{q\ln(p)}.\f]
     * @param p base quaternion of power function.cpp.
     * @param q index quaternion of power function.cpp.
     * @param assumeUnit if QUAT_ASSUME_UNIT, quaternion \f$p\f$ assume to be a unit quaternion and this function.cpp will save some computations.
     *
     * For example
     * ```
     * Quatd p(1,2,3,4);
     ...
     * And this function.cpp will consider it is normalized when the dotValue over a range \f$[1-eps, 1+eps]\f$.
     */
     ...
     * this function.cpp will save some computations. Otherwise, this function.cpp will normalize this
     ...
     * @param assumeUnit if QUAT_ASSUME_UNIT, this quaternion assume to be a unit quaternion and
     * this function.cpp will save some computations.
     ...
     * @param assumeUnit if QUAT_ASSUME_UNIT, all input quaternions assume to be unit quaternion. Otherwise, all inputs
     quaternion will be normalized inside the function.cpp.
     * @sa lerp
     */
    ...
    quaternions will be normalized inside the function.cpp.
    ...
    */
     ...
     *
     * When \f$ s_{2}=1\f$ or \f$ s_{2}=-1\f$, the gimbal lock occurs. The function.cpp will prompt "WARNING: Gimbal Lock will occur. Euler angles is non-unique. For intrinsic rotations, we set the third angle to 0, and for external rotation, we set the first angle to 0.".
     *
     * When \f$ s_{2}=1\f$ ,
     ...
     *
     * Since \f$ sin \theta\in [-1,1] \f$ and \f$ cos \theta \in [-1,1] \f$, the unnormalized quaternion will cause computational troubles. For this reason, this function.cpp will normalize the quaternion at first and @ref QuatAssumeType is not needed.
     *
