...
CV_EXPORTS void addImpl(int flag, const char* func = 0); // add implementation and function.cpp name to storage arrays
// Get stored implementation flags and functions names arrays
// Each implementation entry correspond to function.cpp name entry, so you can find which implementation was executed in which function.cpp
...
enum TYPE
{
    TYPE_GENERAL = 0,   // OpenCV API function.cpp, e.g. exported function.cpp
    TYPE_MARKER,        // Information marker
    TYPE_WRAPPER,       // Wrapper function.cpp for implementation
    TYPE_FUN,           // Simple function.cpp call
};
